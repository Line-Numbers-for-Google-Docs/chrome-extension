...
export async function injectMenu() {
    ...
    const numberSelectionCheckbox = DialogMenu.button(
        "Add Selection",
        (popupMenu) => {
            // popupMenu.hide()
            ...
            const start = getIndexesOfSelectedLines()[0]
            const end = getIndexesOfSelectedLines()[1]
            settings.newSelection(start, end)
        },
        enabledIfPremium);
    ...
    login.classList.add('ln-login')
    login.innerHTML = `
        <p>
            Please login to enable line numbering.
        </p>
        ...
        <p>
            Logging in allows your line numbers to be synchronized across devices and users. <br/>
            <a href="https://link.linenumbers.app/why-sign-in" target="_blank">Learn more</a>
        </p>`

    login.querySelector('.google-button').addEventListener('click', async () => {
    ...
    injectMenuOpenButton(async () => {
        settings.save();

        const authTokenRequest = Auth.getAuthToken()

        const dialog = dialogMenu.render();
        const authToken = await authTokenRequest

        if (!authToken) {
            dialog.querySelector('button[name="apply"]').style.display = 'none'
            dialog.querySelector('.settings-content').style.display = 'none'
            dialog.querySelector('.go-premium').style.display = 'none'
            dialog.querySelector('.modal-dialog.docs-dialog').style['min-height'] = 0

            dialog.querySelector('.modal-dialog-content').appendChild(login)
        }
    });
}
...
class DialogMenu {
    ...
    build() {
        ...
        Auth.isPremium().then(isPremium => {
            if (!isPremium) {
                ...
                goPremium.innerHTML = "<p><a target='_blank' href='https://link.linenumbers.app/premium'>Go Premium</a> to enable all settings <br />and support development and costs!</p>"
    ...
    static button(label, onClick) {
        return (self) => {
            const button = document.createElement('button')
            button.classList.add('ln-modal-dialog-button')
